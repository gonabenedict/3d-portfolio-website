...
import { metalness, roughness } from 'three/src/nodes/TSL.js';
 ...
 document.addEventListener('DOMContentLoaded', () => {
    ...
    const desriptionSplits = new SplitText(".tooltip .description p", {
        type: "lines",
        linesClass: "line",
    });

    header1Split.chars.forEach((char) => (char.innerHTML = `<span>${char.innerHTML}</span>`));
    [...titleSplits.lines, ...desriptionSplits.lines].forEach((line) => (line.innerHTML = `<span>${line.innerHTML}</span>`));
